...
|-------|---------|---------------|
| **Block freshness (#1)** | `curl localhost:26657/status` → check `latest_block_time` vs wall-clock | Within 2 minutes |
| **Data directory readable** | `du -sh ~/.batterycoin-comet/data` | Non-zero, no I/O error |
| **Storage mount state** | `mount \| grep storage` | `rw` (not `ro`) |
| **Block device exists** | `ls -la /dev/sda1` (or relevant device) | Present |
| **Kernel errors** | `dmesg \| grep -iE "ext4-fs error\|i/o error"` | Empty |
...
**Critical:** The **block freshness check** must be first. A node can have SSH up, Tailscale active, and the CometBFT process running — yet be completely dead because its storage failed. This happened to C-3PO (2026-08-12): process alive for 3 days, height frozen for 25 hours, SATA SSD `/dev/sda1` missing from the kernel entirely.
...
- **glibc mismatch for Node Manager upgrades:** HyperCycle Node Manager v0.5.4 is compiled for glibc 2.35 (Ubuntu 22.04+). Boxes on Ubuntu 20.04 (glibc 2.31, e.g. R2-D2) cannot run it — the binary crashes with `version 'GLIBC_2.35' not found`. Always check `ldd --version` before upgrading. See `references/node-manager-v054-update-procedure.md`.
- **USB device claims `/dev/sda` after power outage:** After reboot, a USB storage device may enumerate before the SATA SSD, causing the SSD to shift to `/dev/sdb1`. This breaks `/etc/fstab` entries that hardcode `/dev/sda1`. Fix: use `UUID=` in fstab instead of device paths. See `references/node-manager-v054-update-procedure.md`.
- **SATA SSD failure masquerading as healthy node (C-3PO, 2026-08-12).** A node can pass every superficial check — SSH reachable, Tailscale active, CometBFT process running, RPC responsive — yet be completely dead because its storage device has physically disconnected or failed. The process stays in memory (zombie state) but cannot read or write the database. **Always check `latest_block_time` vs wall-clock before recommending any node for testing.** If the block time is >5 minutes old, dig deeper: check `du -sh ~/.batterycoin-comet/data` (should not be 0 bytes or I/O error), check `mount | grep storage` (should be `rw`, not `ro`), check `ls -la /dev/sda1` (device must exist), and check `dmesg` for `EXT4-fs error`. See `references/c3po-sata-ssd-failure-20260812.md` for full diagnostic transcript.
- **SATA SSD intermittent disappearance pattern (C-3PO, 2026-08-12).** The SSD can vanish from `lsblk`, reappear after reboot as a healthy 1.9TB drive, then corrupt again within minutes. This is hardware failure, not software. See `references/intermittent-sata-ssd-reboot-pattern.md` for reboot-vs-format decision tree and the `mkfs.ext4` agent safety block explanation.
...
- **Node Manager `start_all.sh` `su` password failure:** The HyperCycle Node Manager `start_all.sh` script uses `su -c` to run backend and UI components as the `hypercycle` user. This fails in non-interactive contexts (systemd service, remote SSH, cron) because `su` requires a TTY password prompt. The symptom is a running `hypercycle.service` with no actual processes and no ports listening. The fix is to rewrite `/etc/init.d/hypercycle` to run components directly with `nohup` instead of delegating to `start_all.sh`. See `references/node-manager-v054-update-procedure.md` for the complete init script template.
- **Node Manager glibc mismatch:** v0.5.4 is compiled for glibc 2.35 (Ubuntu 22.04+). Boxes on Ubuntu 20.04 (glibc 2.31, e.g. R2-D2) cannot run it — the binary crashes with `version 'GLIBC_2.35' not found`. Verify `ldd --version` before upgrading. See `references/node-manager-v054-update-procedure.md` for per-OS compatibility.
- **USB device claims `/dev/sda` after power outage:** After reboot, a USB stick may enumerate before the SATA SSD, causing the SSD to shift to `/dev/sdb1`. This breaks `/etc/fstab` entries that hardcode `/dev/sda1`. Fix: use `UUID=` in fstab instead of device paths. See `references/usb-device-ssd-shift-post-power-outage.md`.
...
- `references/intermittent-sata-ssd-reboot-pattern.md` — SATA SSD intermittent disappearance: vanishes from `lsblk`, reappears after reboot, corrupts again. Reboot-vs-format decision tree and `mkfs.ext4` agent safety block.
- `references/node-manager-v054-update-procedure.md` — **Complete guide** for updating HyperCycle Node Manager on HyperAIBox RK3588 ARM64 and x86_64. Covers: architecture mismatch (x86 vs arm64), `start_all.sh` `su` password failure, init script rewrite, `node_modules` copying, `.env` migration, disabling old auto-start mechanisms (including capitalized `Hypercycle.service`), post-reboot old-version respawning, manual component startup, **glibc version compatibility check** (Ubuntu 20.04 vs 22.04), and **x86_64 (AtomMan) update path**.
...
- `references/fleet-monitoring-playbook-cron-20260814.md` — Cron-based fleet sync monitor: ordered check cascade, failure-mode decision tree, validator-state pitfall, and native-CometBFT auto-start gap detection.
