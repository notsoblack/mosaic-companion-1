...
- `references/dual-wallet-anfe-discovery.md` — Discovering ANFEs from multiple wallet sources (Node Manager + Web3) with unified store type, source attribution, and header badge breakdown. Session: 2026-08-17. Core commits `a351011`, `c960820`, `07cd427`, `8945d85`.
- `references/graph-satellite-constellation.md` — Render satellite constellations outside main SVG graph as fixed-position "moons" with orbit rings, hub-satellite connections, and multi-source color attribution. Session: 2026-08-17. Core commit `8945d85`.
- `references/sidebar-action-modal-pattern.md` — **NEW (2026-08-20)**. Store-driven modal triggered from sidebar actions. Step-by-step recipe: add `showXxx`/`setShowXxx` to Zustand store, wire sidebar `onClick` to `getState().setShowXxx(true)`, render modal at top level of CommandCenter shell with `fixed inset-0 z-50` overlay. Includes header, close button, and panel component integration patterns.